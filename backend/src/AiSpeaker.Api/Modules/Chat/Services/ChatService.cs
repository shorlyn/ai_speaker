...
public sealed class ChatService : IChatService
{
    ...
    public async Task<ChatResponse> ProcessTextAsync(ChatTextRequest request, CancellationToken cancellationToken)
    {
        ...
        var audioUrl = await _ttsProvider.GenerateAudioAsync(assistantText, cancellationToken);

        await _conversationService.AddMessageAsync(sessionId, ConversationRoles.Assistant, assistantText, audioUrl, cancellationToken);
